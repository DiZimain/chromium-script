...
for ((i=1; i<=num_profiles; i++)); do
    port=$((base_port + i - 1))
    echo "Профиль $i: https://<IP_сервера>:$port (ВАЖНО: используйте https, не http)"
done
echo "ПРИМЕЧАНИЕ: При первом подключении вам потребуется принять самоподписанный сертификат в браузере."
